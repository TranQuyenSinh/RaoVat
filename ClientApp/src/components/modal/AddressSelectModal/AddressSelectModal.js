...
const AddressSelect = ({ title, className, ...props }) => {
    return (
        <div className={`address-select ${className}`} {...props}>
            <span className={!title && 'text-muted'}>
                {title ? `${title.address}, ${title.ward}, ${title.district}, ${title.province}` : 'Địa chỉ'}
            </span>
            <span>
